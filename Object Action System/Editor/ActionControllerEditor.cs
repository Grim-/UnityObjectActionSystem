...
namespace Grim.ObjectActionSystem
{
    ...
    public class ActionControllerEditor : Editor
    {
        ...
        public virtual void DrawReaction_Name(SerializedProperty reactionToDraw)
        {
            ...
            {
                string reactionName = reactionToDraw.FindPropertyRelative("reactionName").stringValue;
                ...
                {
                    EditorGUILayout.LabelField("Unamed Reaction", boldText);
                }
        ...
        private void DrawAction_Header(SerializedProperty action)
        {
            ...
            EditorGUILayout.LabelField("Action Index [" + currentActionIndex + "]", new GUIStyle("miniLabel"));
        }
        ...
        private void DrawAction_ActionTab(SerializedProperty action)
        {
            ...
            SerializedProperty dataEnumValues = action.FindPropertyRelative("data").FindPropertyRelative("enumValues");
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (objectAction != null)
                {
                    ...
                    PopulateDataValue(dataEnumValues, objectAction, DataValueType.FORCE_MODE_ENUM);
                }
                else
                {
            ...
            if (objectActionProp.objectReferenceValue != null)
            {
                DrawAction_Description((objectActionProp.objectReferenceValue as ObjectAction).description);
            }
        ...
        private void DrawAction_Description(string description)
        {

            GUIContent descriptionText = new GUIContent(description);
            Vector2 size = borderedBox.CalcSize(descriptionText);

            if (!string.IsNullOrEmpty(description))
            {
                var rect = EditorGUILayout.BeginVertical();
                rect.height = size.y;
                EditorGUILayout.LabelField(description, GUILayout.MinHeight(size.y + 6f));
                EditorGUILayout.EndVertical();
            }
        }
        private void DrawAction_DataTab(SerializedProperty action)
        {
            ...
            DrawDataValues(data.FindPropertyRelative("enumValues"), DataValueType.FORCE_MODE_ENUM, objectAction);
            EditorGUILayout.EndVertical();
            serializedObject.ApplyModifiedProperties();
        }
        private void ClearAllDataValues(SerializedProperty data)
        {
            ...
            data.FindPropertyRelative("enumValues").ClearArray();
            ...
            if (dataValue != null)
            {
                switch (type)
                {
                    ...
                    case DataValueType.SCRIPTABLE_OBJECT:
                        ...
                        break;
                    case DataValueType.FORCE_MODE_ENUM:
                        dataValue.arraySize = obAction.enumValues.Count;
                        for (int i = 0; i < obAction.enumValues.Count; i++)
                        {
                            SerializedProperty soo = dataValue.GetArrayElementAtIndex(i);
                            soo.FindPropertyRelative("name").stringValue = obAction.enumValues[i].name;
                            soo.FindPropertyRelative("value").enumValueIndex = (int)obAction.enumValues[i].value;
                        }
                        break;
                }
            }
        ...
        private void DrawDataValues(SerializedProperty dataValue, DataValueType type, ObjectAction objectAction)
        {
            for (int i = 0; i < dataValue.arraySize; i++)
            {
                ...
                DrawDataValue(dataValue.GetArrayElementAtIndex(i), type, objectAction);
        ...
        private void DrawDataValue(SerializedProperty dataValue, DataValueType type, ObjectAction action)
        {
            switch (type)
            {
                case DataValueType.FLOAT:
                ...
                case DataValueType.SCRIPTABLE_OBJECT:
                    ...
                    break;
                case DataValueType.FORCE_MODE_ENUM:
                    EditorGUILayout.PropertyField(dataValue.FindPropertyRelative("value"), GUIContent.none);
                    break;
            }
        }
        private void DrawCurrentObjectActionLabel()
        {
            ...
            if (controller !=null && controller.GetCurrentAction() != null && showDebugInScene)
            {
